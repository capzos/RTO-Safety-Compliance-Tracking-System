...
└────────────┬────────────────┘
             ...
             ▼
┌─────────────────────────────┐
│  Custom CNN Classifier      │  ← Red reflector present?
│  (TensorFlow / PyTorch)     │
└────────────┬────────────────┘
             │
     ┌───────┴───────┐
...
- Cropped tractor image is passed to a custom-trained CNN
...
| Classification | TensorFlow / PyTorch CNN | CNNs excel at visual pattern detection; custom-trained on domain-specific images |
...
### CNN — Reflector Classifier

- Architecture: Custom CNN (Conv → Pool → Conv → Pool → Dense → Sigmoid)
